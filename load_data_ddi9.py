...
train_filename = 'train9.tsv'
test_filename = 'test9.tsv'
...
# Chequea si los archivos de datos existen
...
def load_sentence(filename, max_sent_len):
    ...
    with open(os.path.join(data_dir, filename), 'r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            #print("mcjh1", item_lst[1] )
            drug2_lst.append(item_lst[4])
            #print("mcjh4", item_lst[4])
            rel_lst.append(rel_class[item_lst[7]])
            #print("mcjhR", item_lst[7])
            sent = item_lst[8].split(' ')
            #print("mcjhSent", sent)
            # Antes del remplazo,  onstruir la lista de posiciones
...
def build_position_vocab(pos_lst):
    ...
    # d1 y d2 deben llamarse por separado
    #print("build pos lst", pos_lst)
    sent_list = sum(pos_lst, [])
    #print("build1", sent_list)
    wf = {}
...
def build_word_vocab(sentences):
    ...
    with open(os.path.join(data_dir, vocab_filename), 'w', encoding='utf-8') as f:
        for w, idx in vocb.items():
            f.write("{}\t{}\n".format(idx, w))
    #print("mcjh vocb ",vocb)
    #print("mcjh vocb_inv ",vocb_inv)
...
def word2idx(sentences, vocb, unk_limit):
    ...
    for sentence in sentences:
        ...
        for word in sentence:
            ...
            #print("mcjh word_idx ", w_id)
            #print("mcjh sentence ", sentence)
    ...
    for word, i in vocb.items():
        if i < unk_limit:
            try:
                ...
                # word2vec Palabra no encontrada en
...
def load_data(unk_limit, max_sent_len, dev_size):
    ...
    print('mcjh word2idx:', tr_sentences2idx)
    print('mcjh word2idx:', te_sentences2idx)
    ...
    print('mcjh seq:', tr_sentences2idx)
    print('mcjh seq:', te_sentences2idx)

    # numpy to list
    tr_sentences2idx = tr_sentences2idx.tolist()
    te_sentences2idx = te_sentences2idx.tolist()
    print('mcjh a lista:', tr_sentences2idx)
    print('mcjh a lista:', te_sentences2idx)

    # tr_y, te_y
    tr_y = one_hot_encoding(tr_rel_lst)
    te_y = one_hot_encoding(te_rel_lst)
    print('mcjh tr_y[0]:', tr_y)
    print('mcjh te_y[0]:', te_y)
...
def to_piece(sequence, pos_tuple_lst):
    ...
    for i in range(len(sequence)):
        ...
        #print("mcjh left_idx", left_idx)
        right_idx = pos_tuple_lst[i][1]
        #print("mcjh right_idx", left_idx)
...
def sentence_split_for_pcnn(sentences2idx, d1_pos_lst, d2_pos_lst, pos_tuple_lst, max_sent_len):
    ...
    '''
    # Split into 3 parts
    #print("mcjh sentences2idx", sentences2idx)
    ...
    print("sent_left: ", d1_left)
    print("sent_mid: ", d1_mid)
    print("sent_right: ", d1_right)
