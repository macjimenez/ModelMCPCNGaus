...
class CNN(object):
    ...
    def add_fc_layer(self):
        ...
        self.fc_l = Dense(10, kernel_regularizer=l2(self.reg_coef_dense))(self.fc_l)
        self.fc_l = Dense(1, kernel_regularizer=l2(self.reg_coef_dense))(self.fc_l)

        if self.use_batch_norm:
            self.fc_l = BatchNormalization()(self.fc_l)
### Aumento codigo antes de l activacion ruido gausiano
        self.fc_l  = GaussianNoise(stddev=0.15)(self.fc_l)
    ...
    def train(self, train_data, dev_data):
        ...
        # Write log
        logging.basicConfig(filename=os.path.join(result_dir, 'result2.log'),
...
class BILSTM(CNN):
    ...
    def add_rnn_layer(self):
        ...
        self.rnn_l = SeqSelfAttention(units=32,
                                        attention_activation='sigmoid',
                                        kernel_regularizer=l2(1e-6),
                                        bias_regularizer=l1(1e-6),
                                        attention_regularizer_weight=1e-6)(self.rnn_l)
...
### Aumento codigo antes de l activacion ruido gausiano
        self.rnn_l   = GaussianNoise(stddev=0.15)(self.rnn_l)
...
class PCNNVar(PCNN):
    ...
    def add_cnn_layer(self):
        ...
        self.emb_concat_left = Bidirectional(LSTM(self.rnn_dim, dropout=self.dropout_rate,
                                        recurrent_dropout=self.dropout_rate, return_sequences=True))(self.emb_concat_left)
        
        self.emb_concat_left = SeqSelfAttention(units=32,
                                        attention_activation='sigmoid',
                                        kernel_regularizer=l2(1e-6),
                                        bias_regularizer=l1(1e-6),
                                        attention_regularizer_weight=1e-6)(self.emb_concat_left)
        self.emb_concat_mid = Bidirectional(LSTM(self.rnn_dim, dropout=self.dropout_rate,
                                        recurrent_dropout=self.dropout_rate, return_sequences=True))(self.emb_concat_mid)
        
        self.emb_concat_mid = SeqSelfAttention(units=32,
                                        attention_activation='sigmoid',
                                        kernel_regularizer=l2(1e-6),
                                        bias_regularizer=l1(1e-6),
                                        attention_regularizer_weight=1e-6)(self.emb_concat_mid)
        self.emb_concat_right = Bidirectional(LSTM(self.rnn_dim, dropout=self.dropout_rate,
                                        recurrent_dropout=self.dropout_rate, return_sequences=True))(self.emb_concat_right)
        
        self.emb_concat_right = SeqSelfAttention(units=32,
                                        attention_activation='sigmoid',
                                        kernel_regularizer=l2(1e-6),
                                        bias_regularizer=l1(1e-6),
                                        attention_regularizer_weight=1e-6)(self.emb_concat_right)
...
class MC_PCNN_ATT(MC_PCNN):
    def add_cnn_layer(self):
        ...
        for kernel_size in self.kernel_lst:
            ...
            # TODO: Mecanismo de Atención a cada ventana o en el estado de [Ninguno, 3, 400] donde todas las ventanas están concatenadas
            #conv_concat = SeqSelfAttention(units=32, attention_activation='sigmoid')(conv_concat)  # Base self attention
            
            #conv_concat  = Attention()(conv_concat)
